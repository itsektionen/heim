...
const YearHeader = ({ year }: { year: string }) => {
  return (
    <CollapsibleTrigger className="border-b sm:border-y bg-muted/90 hover:bg-accent transition-colors backdrop-blur z-0 sticky top-0 sm:top-16 px-6 py-3">
      <h2 className="text-sm font-medium select-none">{year}</h2>
...
const ProtocolsPage = async ({
  ...
  return (
    <div className="-ml-6 -mt-6 -mr-6 -mb-42 flex flex-col min-h-[700px]">
      <section className="px-6 py-3 text-sm">
        <h1 className="flex items-center gap-2 mb-1 font-medium">
      ...
      <section>
        {allMeetings.map((meeting, i) => (
          <Collapsible
            ...
            open={i === 0}
            key={meeting.id}>
            <YearHeader year={meeting.year!} />
            <CollapsibleContent>
              <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-6 gap-4">
                {meeting.files?.map((file) => (
                  <li key={file.id}>
                    <Card className="flex h-full select-none flex-row overflow-hidden">
                      {file.hasThumbnail ? (
                        <Image
                          height={138}
                          width={100}
